...
def special_key_listener(key, x, y):
    ...
    if camera_mode == "follow":
        ...
        if key == GLUT_KEY_LEFT:
            ...
            camera_pos[1] -= 10
    camera_pos[0] = max(-GRID_SIZE/2 + 50, min(GRID_SIZE/2 - 50, camera_pos[0]))
    camera_pos[1] = max(-GRID_SIZE/2 + 50, min(GRID_SIZE/2 - 50, camera_pos[1]))
    camera_pos[2] = max(-(current_level * level_depth) + 50, min(-50, camera_pos[2]))
